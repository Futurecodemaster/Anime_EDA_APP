...
df = load_data()  # Reusing the load_data function from the previous snippet

# Converting score column to numeric for analysis
...
df = load_data()  # Reusing the load_data function
...
st.markdown("""
- **Up next, let's dive into the distribution of different types of anime, such as TV series, movies, and others.**
...
st.markdown("""
- **Now, let's delve into the range of episode counts across the dataset.**
- **This analysis is designed to shed light on the typical length of anime series**, helping us understand how many episodes are common for different types of shows.
- **Understanding episode count distributions will offer valuable insights** into the storytelling and production norms in the anime industry.
""")

# Convert 'Episodes' to numeric, setting errors to NaN
...
def main():
    ...
    # Transpose the stats for horizontal display
...
st.markdown(""" 
- **We're now shifting our focus to the temporal distribution of anime production.**
- **This involves a detailed examination of the 'Aired' and 'Premiered' columns** in our dataset, which will reveal how anime release patterns have evolved over time.
...
# Plotting the trend of anime airing over years
...
- **Key Observation**: There's a noticeable general increase in the number of anime premiering each year, with a significant uptick starting from the early 2000s. This trend mirrors the anime industry's expansion and its escalating popularity across the globe.
- **Noteworthy Fluctuations**: The plot also highlights some fluctuations in certain years. These variations could be reflections of changing industry dynamics, broader economic conditions, or even nuances in how data has been collected over time.
...
# Convert 'Popularity' to numeric for analysis
df['Popularity'] = pd.to_numeric(df['Popularity'], errors='coerce')

# Grouping data by aired year and calculating mean popularity
popularity_trends = df.groupby('Aired Year')['Popularity'].mean().dropna()

# Plotting the trend of average popularity over years
plt.figure(figsize=(15, 6))
sns.lineplot(x=popularity_trends.index, y=popularity_trends.values)
plt.title('Trend of Average Anime Popularity Over Years')
plt.xlabel('Year')
plt.ylabel('Average Popularity')
plt.gca().invert_yaxis()  # Inverting y-axis as lower numbers indicate higher popularity
st.pyplot(plt)
...
st.title("Anime Genre Analysis")
st.markdown("""
- **Now let's explore the Favorite Genres** by analyzing which genres are most favored by the audience. 
- **This analysis will involve examining the 'Favorites' column** in relation to the genres, providing insights into which genres resonate the most with viewers.
- **Understanding audience preferences in genres** can help in discerning trends and patterns in anime popularity.
""")
...
all_genres = []
for genres in anime_data['Genres']:
...
top_10_genres_transposed = top_10_genres.T
st.write(top_10_genres_transposed)

# Plotting
plt.figure(figsize=(12, 8))
sns.barplot(x=genre_df['Count'], y=genre_df.index, palette="viridis")
plt.title('Favorite Genres in Anime')
plt.xlabel('Count')
plt.ylabel('Genre')
plt.grid(True)
...
- **Comedy** leads the list as the most prevalent genre with 6029 occurrences, indicating its widespread popularity among anime viewers.
- **Action** follows as the second most popular genre with 3888 occurrences, showcasing the audience's inclination towards dynamic and thrilling content.
- **Fantasy** and Adventure are also highly favored, with 3285 and 2957 occurrences respectively. These genres often offer imaginative and escapade-rich narratives.
- **Kids** and Music genres have a significant presence as well, suggesting a diverse audience base including younger viewers and those who enjoy musical elements in anime.
- Other popular genres include Drama, Sci-Fi, Shounen (targeted towards young male demographic), and Slice of Life. These genres reflect varied themes from realistic depictions of everyday life to futuristic and science fiction settings.
- While Comedy, Action, and Fantasy dominate, genres like Psychological or Horror, though less prevalent, cater to a niche audience seeking depth and intensity. This variation highlights the diverse tastes within the anime community.
""")
...
# Drop rows where either 'Score' or 'Members' is NaN
...
# Streamlit: Display the plotly plot
st.plotly_chart(fig)
...
# Drop rows where either 'Score' or 'Episodes' is NaN
...
### Key Observations
- **Short Series Variability**: A significant number of anime have a low number of episodes, with a wide range of scores. This cluster indicates that short series can vary greatly in quality.
- **Long Series Analysis**: There are fewer anime with a very high episode count. While some of these longer series have high scores, the correlation is not strong enough to suggest a definitive trend.
- **Influence of Episode Count**: The weak correlation implies that the number of episodes is not a significant factor in determining the quality or appeal of an anime. Other factors like story, character development, and production quality might play more pivotal roles.
...
# Generating and displaying the heatmap
